...
const FriendRequestItem = ({ requester, onFriendAction }) => {
  ...
  const [
    acceptInvitation,
    { isLoading: acceptinvitationisloading, error: acceptinvitationerror }
  ] = useAcceptInvitationMutation();
  const [
    declineInvitation,
    { isLoading: declineinvitationisloading, error: declineinvitationerror }
  ] = useDeclineInvitationMutation();
  ...
  return (
    <View style={styles.container}>
      {isUrl ? (
        ...
        <Image
          source={{ uri: `data:image/png;base64,${requester.image}` }}
          style={styles.image}
        />
      )}
