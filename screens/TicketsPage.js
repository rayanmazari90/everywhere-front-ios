...
const TicketsPage = () => {
    ...
    const [executeMutation, { data, isMutating, isError }] = useTicketsByUserPostMutation();
    useEffect(() => {
        if (ticket && user_ && event) {
            console.log("wait a minute ", ticket._id)
          executeMutation({ eventId: event._id, userId: user_.user._id , ticketId:ticket._id})
            .then(result => {
    ...
    return (
        ...
        {ticketStatus ?
            ...
            <Animated.View style={{ opacity: fadeAnim }}>
                <MaterialCommunityIcons name="check-circle-outline" size={50} color="green" />
            </Animated.View>
            <Animated.View style={{ opacity: fadeAnim }}>
            <Text style={{ textAlign: 'center', fontSize: 16, marginTop: 20 }}>Good job! You got your ticket! Now get ready to party!</Text>
            </Animated.View>
            </>
            :
            <>
            <Animated.View style={{ opacity: fadeAnim }}>
                <MaterialCommunityIcons name="close-circle-outline" size={50} color="red" />
            </Animated.View>
            <Animated.View style={{ opacity: fadeAnim }}>
            <Text style={{ textAlign: 'center', fontSize: 16, marginTop: 20 }}>Sorry, there is no more ticket left for you! </Text>
            <Text style= {{ textAlign: 'center', fontSize: 16, marginTop: 10 }}>Try next time!</Text>
            </Animated.View>
            </>
        }
        </View>
