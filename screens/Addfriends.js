...
const AddFriends = ({ navigation }) => {
  ...
  const AddFriendsScene = React.memo(() => {
    ...
    const [
      getRequesters,
      { isLoading: getRequestersIsLoading, error: getRequestersError }
    ] = useGetRequestersMutation();
    const [
      getusersnotfriends,
      { isLoading: getusersnotfriendsIsLoading, error: getusersnotfriendsError }
    ] = useGetUsersNotFriendsMutation();
    ...
    const fetchusersnotfriends = async () => {
      try {
        const result = await getusersnotfriends({ userId: user.user._id });
        console.log(result);
        const users = result.data.users;
        setAllUsers(users);
      } catch (error) {
      ...
      <View style={{ flex: 1 }}>
        ...
        {filteredUsers[0] ? (
          <FlatList
            data={filteredUsers}
            renderItem={({ item }) => (
              <AddfriendItem friend={item} onFriendAction={onFriendAction} />
            )}
...
/*const fetchGroups = async () => {
    fetch("http://192.168.56.1:5001/getusersandnotfriends")
        .then((res) => res.json())
