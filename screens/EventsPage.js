...
const CountdownEvent = ({ eventDate }) => {
  ...
  return (
    <View style={{}}>
      <Text style={styles.sectionTitle}>Time left Before Party!</Text>
      <View
        style={{
          alignItems: "center",
...
const EventsPage = () => {
  ...
  const renderTickets = (ticket, event) => {
    const { description, numberOfTicketsLeft, ticketType, timeofexpir } =
      ticket;
  ...
  const CountdownTickets = ({ eventDate, expirationHours, onExpire }) => {
    const calculateCountdown = () => {
      ...
      const expirationTime = new Date(
        eventTime.getTime() - expirationHours * 60 * 60 * 1000
      ); // Subtract the hours
    ...
    if (countdown.isExpired) {
      return <Text>Ticket expired</Text>;
    }
            ...
            >
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                >
                  {" "}
                  Event Name:{" "}
                  <Text style={{ padding: 0, color: "black" }}>
                    {event.eventName}
                  </Text>
                </Text>
              </View>
                  ...
                  <Text style={{ fontWeight: "bold", color: darkGreen }}>
                    ...
                    <Text style={{ padding: 0, color: "black" }}>
                      {tickets
                        ? tickets.reduce(
                            (acc, ticket) =>
                              acc + ticket.users_having_ticket.length,
                            0
                          )
                        : "Loading..."}
                    </Text>
              ...
              <View style={{ marginTop: 10 }}>
                ...
                {tickets &&
                  tickets.map((ticket) => renderTickets(ticket, event))}
              </View>
            </View>
          </View>
        </View>
