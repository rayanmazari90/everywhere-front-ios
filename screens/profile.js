...
const Profile = () => {
  ...
  const [logoutUser, { logoutisLoading, logouterror }] =
    useLogoutUserMutation();
  const [isLoading, setIsLoading] = useState(true);
  const [userInfo, setUserInfo] = useState(null);
  const [resourcePath, setResourcePath] = useState(null);
  const [updateUserImage, { isLoading: isUploading, error: uploadError }] =
  ...
  const {
    ...
    isError,
    refetch
  } = useUserInfoProfilePageGetQuery(user._id || null, {
    enabled: !!user._id
  });
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    loadUserInfo(user._id).then(() => setRefreshing(false));
    // Any additional API calls should go here.
  }, [user]);
        ...
        >
          ...
          <View style={{ alignSelf: "center" }}>
            <View style={styles.profileImage}>
              {userInfo && userInfo.image ? (
                <Image
                  source={{ uri: `data:image/png;base64,${userInfo.image}` }}
                  style={styles.image}
                  resizeMode="center"
                ></Image>
              ) : (
            ...
            <View style={styles.dm}>
              {/*<MaterialIcons MaterialIconsname="chat" size={18} color="#DFD8C8"></MaterialIcons>*/}
            </View>
          ...
          <View style={styles.statsContainer}>
            ...
            <Pressable
              ...
              onPress={() => navigation.navigate("Removefriends")}
            >
              <Text style={[styles.text, { fontSize: 24 }]}>
          ...
          <View style={{ marginTop: 32 }}>
            ...
            >
              {userInfo &&
              userInfo.historyoftickets &&
              userInfo.historyoftickets.length > 0 ? (
