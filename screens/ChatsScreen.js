...
const ChatsScreen = () => {
  ...
  const [getConvs, { isLoading: coonvsisloading, error: convserror }] =
    useGetconvsMutation();
  const [getGroups, { isLoading: groupsisloading, error: groupserror }] =
    useGetGroupsMutation();
  ...
  // Add this useEffect to listen for the 'updateConversations' event
  useEffect(() => {
      ...
      {conversations.length > 0 ? (
        ...
        <View style={styles.chatemptyContainer}>
          <Text style={styles.chatemptyText}>
            IE is all about making friends! Add friends to chat with them!
          </Text>
